...
class SmokeWorkflowTests(unittest.TestCase):
    ...
    def test_smoke_script_exercises_end_to_end_pipeline_outputs(self) -> None:
        ...
        self.assertIn('run_prepared_max_quality_pipeline "$BASE_DATA" 1', text)
        ...
        self.assertIn("RUN_HARD_MINING=0", text)
        self.assertIn("RUN_DISTILL=0", text)
        self.assertIn("ENS_FIT_STEPS=10", text)
        ...
        self.assertNotIn("scripts/fit_ensemble.py", text)
        self.assertNotIn("scripts/fit_domain_thresholds.py", text)
        self.assertNotIn("scripts/eval_test_ensemble.py", text)
        self.assertNotIn("scripts/export_best_release.py", text)
