...
class DocsSurfaceTests(unittest.TestCase):
    ...
    def test_readme_points_to_split_startup_docs(self) -> None:
        ...
        self.assertLess(text.index("## Secure Linux VM + Docker Compose"), text.index("## Dependency Profiles"))
    ...
    def test_startup_doc_marks_sudo_only_for_system_commands(self) -> None:
        ...
        self.assertIn("Best security with GPU", text)
        self.assertIn("GPU passthrough", text)
        self.assertIn("source checkout at `/workspace` read-only", text)
        self.assertLess(text.index("## Dedicated Linux VM + Docker Compose startup"), text.index("## Native Linux startup"))
    ...
    def test_commands_doc_starts_with_linux_quick_start(self) -> None:
        ...
        self.assertIn("command snippets in this file use Linux `bash` syntax", text)
        ...
        self.assertIn("Docker Compose is not a real VM.", text)
        self.assertIn("You do not need host Python, `pip`, or a host `./.venv` for this secure path.", text)
