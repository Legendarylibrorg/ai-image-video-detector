...
class DocsSurfaceTests(unittest.TestCase):
    ...
    def test_readme_points_to_split_startup_docs(self) -> None:
        ...
        self.assertIn("docker compose run --rm pipeline ./local.sh deps", text)
        ...
        self.assertIn("reduces blast radius", text)
        self.assertIn("does not make malicious packages harmless", text)
        self.assertLess(text.index("## Docker Compose"), text.index("## Quick Start"))
    ...
    def test_startup_doc_marks_sudo_only_for_system_commands(self) -> None:
        ...
        self.assertIn("dependency install happens inside the container", text)
        self.assertIn("does not guarantee safety from malicious packages", text)
        self.assertLess(text.index("## Docker Compose startup"), text.index("## Native Linux startup"))
    ...
    def test_commands_doc_starts_with_linux_quick_start(self) -> None:
        ...
        self.assertIn("dependency install happens inside the container", text)
        self.assertIn("does not make malicious packages safe", text)
        self.assertLess(text.index("## Docker Compose commands"), text.index("Clone path:"))
        self.assertIn("Linux-native command blocks", text)
