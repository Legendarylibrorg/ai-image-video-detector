...
class DocsSurfaceTests(unittest.TestCase):
    ...
    def test_docker_compose_splits_cpu_and_gpu_images(self) -> None:
        ...
        self.assertNotIn("HF_HUB_ENABLE_HF_TRANSFER", compose_text)
        self.assertNotIn("HUGGINGFACE_HUB_CACHE", compose_text)
    ...
    def test_readme_points_to_split_startup_docs(self) -> None:
        ...
        self.assertIn("## Linux First Start", text)
        self.assertIn("curl -fsSL -o ai-image-video-detector.tar.gz", text)
        self.assertIn("Hugging Face `read` token", text)
        self.assertIn("`hf auth login` also works", text)
        self.assertIn("optional Python dev", text)
        ...
        self.assertNotIn("HUGGINGFACE_HUB_TOKEN", text)
    ...
    def test_startup_doc_marks_sudo_only_for_system_commands(self) -> None:
        ...
        self.assertIn("`hf auth login` is also supported", text)
    ...
    def test_commands_doc_starts_with_linux_quick_start(self) -> None:
        ...
        self.assertIn("## macOS and Windows (short)", text)
