...
class TrainingSurfaceTests(unittest.TestCase):
    ...
    def test_reference_doc_stays_pipeline_focused(self) -> None:
        ...
        self.assertIn("Pipeline architecture (data flow)", text)
        self.assertIn("flowchart TB", text)
        self.assertIn("`scripts/*.py` inventory", text)
        self.assertIn("build_best_dataset.py", text)
        self.assertIn("release_selection.py", text)
