...
- **CI alignment:** the local quality gate uses **`.github/ci-python-version.txt`**; **`scripts/update_deps_lock.py`** uses **`MANIFEST_MAX_WHEEL_CP`** so **`requirements.lock.json`** wheel tags stay in step with that interpreter (see [REFERENCE.md](REFERENCE.md) and [CI_LOCAL.md](CI_LOCAL.md)).
