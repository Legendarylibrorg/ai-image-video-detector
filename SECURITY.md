...
- **Python 3.11+** (**.github/ci-python-version.txt** pins the lock refresh interpreter; **`MANIFEST_MAX_WHEEL_CP`** in **`scripts/update_deps_lock.py`** must match that minor for wheel-tag selection). **`requirements.lock`** + **`requirements.lock.json`** are authoritative pins (per-artifact **SHA256**, verified vs PyPI via **`make ci-fast`** / **`scripts/run_ci_local.py`**); **markdown is not**. The JSON records **one** PyPI file per package—another OS may still install the same **version** from a different wheel; regenerate on **Linux x86_64** (or the lock-refresh workflow) if you need the manifest to match CI.
